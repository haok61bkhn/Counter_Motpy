...
class Tracker:
    def __init__(
            ...
            smooth_feature_gamma: float = 0.9,box_cur=[],trace=[]):
...
class MultiObjectTracker:
    ...
    def active_tracks(self,
        ...
        for tracker in self.trackers:
            ...
            if cond1 and cond2 and cond3:
                tracks.append(Track(id=tracker.id, box=tracker.box, score=tracker.score,box_cur=tracker.box_cur,trace=tracker.trace))
    ...
    def step(self, detections: Sequence[Detection]) -> Sequence[Track]:
        ...
        for det_idx in set(range(len(detections))).difference(assigned_det_idxs):
            tracker = Tracker(box0=detections[det_idx].box,box_cur=detections[det_idx].box,
