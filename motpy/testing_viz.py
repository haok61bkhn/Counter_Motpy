...
def draw_trace(image,trace,color):
    for pos in trace:
        image = cv2.circle(image, pos, 1, color, 3)
    return image

def draw_track(img, track: Track,random_color: bool = True, fallback_color=(255, 255, 255)):
    ...
    img=draw_trace(img, track.trace,color)
